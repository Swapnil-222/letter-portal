...
export class MenuService {
  ...
  getMenuById(menuId: number): Menu {
    const objIndex = this.menu$.value.findIndex(obj => obj.id == menuId);
    return this.menu$.value[objIndex];
  }
  ...
  /** Initialize the menu data. */
  update(menu: Menu){
    const objIndex = this.menu$.value.findIndex((obj => obj.id == menu.id));


    this.menu$.value[objIndex]=menu;
